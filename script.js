...
// Service Cards Scatter Animation on Scroll
document.addEventListener('DOMContentLoaded', () => {
        ...
        entries.forEach(entry => {
            if (entry.isIntersecting && !animated) {
                ...
                serviceCards.forEach((card, index) => {
                    // Calculate scatter positions (0-360 degrees around center)
                    const angle = (index / serviceCards.length) * Math.PI * 2;
                    const distance = 80 + (Math.random() * 40); // 80-120px scatter distance
                    ...
                    card.style.setProperty('--scatter-transform', `translate(${tx}px, ${ty}px)`);
                    
                    // Delay animation for each card
                    card.style.animationDelay = `${index * 0.1}s`;
                    card.classList.add('scatter-enter');
                    
                    // After scatter animation completes, start float animation
                    setTimeout(() => {
                        card.style.setProperty('--scatter-transform', `translate(0, 0)`);
                        card.style.animation = `floatAnimation 4s ease-in-out infinite`;
                    }, 800 + (index * 100));
                });
    ...
    const servicesSection = document.querySelector('.services-overview, .services');
    if (servicesSection) {
